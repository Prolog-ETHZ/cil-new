...
# Load The Labels -> y
train_labels = extract_labels('./groundtruth/', TRAIN_SIEZ)
#print(train_labels.shape)
# Form The Input -> x
predicted_labels = extract_predictions('./predictions/', TRAIN_SIEZ)
...
for image in predicted_labels:
    for i, _ in enumerate(image):
        for j, _ in enumerate(image[i]):
            ...
            while row_span > 0:
                row_span -= 1
                data.append(image[row_span][j] if )
            row_span = i
            # Add Down
            while row_span < int(TRAIN_WIDTH / IMG_PATCH_SIZE) - 1:
                row_span += 1
                data.append(image[row_span][j])
            # Add Left
            while col_span > 0:
                col_span -= 1
                data.append(image[i][col_span])
            col_span = j
        

            # Add Right
            while col_span < int(TRAIN_WIDTH / IMG_PATCH_SIZE) - 1:
                col_span += 1
                data.append(image[i][col_span])
            '''
            # Left Right Up Down for 5 step
...
with tf.Session() as s:
    ...
    # Now Prediction
    p_dir = '../predictions_training/'
    s_dir = './post_result/'
    for i in range(1,PREDICTION_SIZE+1):
        filename = 'prediction_'+str(i)+'.png'
